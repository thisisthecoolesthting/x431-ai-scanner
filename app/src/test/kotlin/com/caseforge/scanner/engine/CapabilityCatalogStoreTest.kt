...
class CapabilityCatalogStoreTest {
    ...
    fun setUp() {
        ...
        every { assetManager.open("capabilities/capabilities.json") } answers {
            ByteArrayInputStream(
                json.encodeToString(CapabilityCatalog.serializer(), baselineMap).toByteArray()
            )
        }
    ...
    private fun registry(
        cacheContents: CapabilityCatalog? = null,
        ...
        if (cacheContents != null) {
            ...
            cacheFile.writeText(json.encodeToString(CapabilityCatalog.serializer(), cacheContents))
    ...
    fun cacheMiss_remoteSuccess_returnsMergedMap() = runTest {
        ...
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setBody(json.encodeToString(CapabilityCatalog.serializer(), remoteMap))
        )
    ...
    fun merge_baselineFillsMissingIds() = runTest {
        ...
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setBody(json.encodeToString(CapabilityCatalog.serializer(), remoteWithoutBaseline))
        )
