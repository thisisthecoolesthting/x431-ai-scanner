...
object VciConnectionDiagnostics {
    ...
    suspend fun runChain(
        ...
        step(
            ...
            VciProtocolConfig.protocolStatusLine(settings),
        )
        if (settings.directVciExperimental && !settings.vciProtocolConfirmed) {
            step(
                "Protocol probe",
                false,
                "Run Direct VCI probe to confirm header + transport before relying on OEM path",
            )
        }

        if (mode != VciConnector.Mode.BLUETOOTH) {
            val usbClient = OemUsbVciClient(
                context,
            ...
            if (tryLiveConnect && devices.isNotEmpty()) {
                ...
                if (!usbClient.hasPermission(dev)) {
                    ...
                    val usbTimeoutMs = OemUsbVciClient.OPEN_DEVICE_TIMEOUT_MS +
                        OemUsbVciClient.socketTimeoutFor(settings.vciProtocolConfirmed) + 2_000L
                    val usbResult = runCatching {
                        withTimeout(usbTimeoutMs) { usbClient.connect(dev) }
                    }
                    usbResult.fold(
                        onSuccess = { r ->
                            r.fold(
        ...
        if (!settings.bluetoothTransportEnabled) {
            step(
                ...
                true,
                "skipped — Bluetooth transport disabled in settings",
            )
            return steps
        }
        ...
        if (!settings.bluetoothTransportEnabled) {
            step("BLE fallback", true, "skipped — Bluetooth transport disabled in settings")
            return steps
        }

        // BLE fallback probe (opt-in via Bluetooth transport enabled)
