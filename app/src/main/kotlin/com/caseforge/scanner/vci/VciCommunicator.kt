 ...
 *   actuate()    ↔ Proprietary active test (gated; framed request/response)
 *
 * SPIKE LIMITATIONS:
 ...
 *   - Actuate() requires Direct VCI experimental + approval token + Ford/Stellantis marque.
 *
 * THREADING:
...
class VciCommunicator(
    ...
    /** Expected frame header — defaults to [VciProtocolConfig.header]. */
    private val expectedHeader: ByteArray = VciProtocolConfig.header.copyOf(),
    /** When true, reject frames failing checksum or header match (Phase A framing). */
    private val requireValidFraming: Boolean = true,
) {
    companion object {
        ...
        /**
         * Fallback host→VCI opcode for raw CAN shim (4-byte BE CAN ID + 8-byte data).
         * Matches [com.caseforge.scanner.obd.VciObdTransport.SHIM_CAN_RAW_OPCODE].
         */
        const val SHIM_CAN_RAW_OPCODE: Int = 0x0200

        /**
         * Inferred active-test response opcode (request [KnownOpcode.PROPRIETARY_ACTIVE_TEST] | 0x80).
         * Unverified on wire — treat [ActuateStatus.PARTIAL] when payload is ambiguous.
         */
        const val ACTIVE_TEST_RESP_OPCODE: Int = 0x0089

        /** Actuate request payload version byte (v1: [version][len][testId…]). */
        const val ACTUATE_PAYLOAD_VERSION: Int = 0x01

        /** Shared with [VciProtocolConfig] probe classification (no transport instance needed). */
        fun parseDtcPayloadStatic(payload: ByteArray): List<Dtc> {
            ...
            while (i + 1 < payload.size) {
                ...
                val code =
                    "$systemLetter$digit1$digit2${digit3.toString(16).uppercase()}${digit4.toString(16).uppercase()}"
    ...
    fun isTransportConnected(): Boolean = false

    fun sendRoutedCanFrame(canId: Int, data: ByteArray): Result<Unit> = Result.success(Unit)

    suspend fun awaitRoutedCanData(canId: Int): Result<ByteArray> = Result.success(ByteArray(0))

    internal fun packCanShim(canId: Int, data8: ByteArray): ByteArray {
        require(data8.size == 8) { "CAN data must be 8 bytes, got ${data8.size}" }
    ...
    internal fun unpackCanShim(payload: ByteArray): Pair<Int, ByteArray>? {
        ...
        return canId to payload.copyOfRange(4, 12)
    }

    private suspend fun awaitRoutedCanDataInternal(respCanId: Int): ByteArray? {
        return try {
            withTimeout(requestTimeoutMs) {
                transport.frames
                    .mapNotNull { frame ->
                        if (frame.opcode != SHIM_CAN_RAW_OPCODE) return@mapNotNull null
                        val (id, data) = unpackCanShim(frame.payload) ?: return@mapNotNull null
                        if (id != respCanId) return@mapNotNull null
                        data
                    }
                    .firstOrNull()
            }
        } catch (_: kotlinx.coroutines.TimeoutCancellationException) {
            Log.w(TAG, "Timeout waiting for routed CAN 0x${respCanId.toString(16)}")
            null
        }
    }

    private fun normalizeCanData(payload: ByteArray): ByteArray {
        if (payload.size >= 8) return payload.copyOf(8)
        return ByteArray(8).also { out ->
            payload.copyInto(out, 0, 0, payload.size)
            for (i in payload.size until 8) {
                out[i] = 0xCC.toByte()
            }
        }
    }
    ...
    suspend fun readDtcs(): Result<List<Dtc>> = safeRequest("readDtcs") {
        ...
        val responseFrame = awaitResponse(KnownOpcode.OBD_MODE03_DTC_RESP)
            ?: return@safeRequest Result.failure(
                VciException.Timeout(
                    KnownOpcode.OBD_MODE03_DTC_REQ,
                    requestTimeoutMs,
                    remediation = "No valid Mode 03 frame — confirm header ${VciProtocolConfig.headerLabel(expectedHeader)} " +
                        "and transport mode via Direct VCI probe.",
                ),
            )
    ...
    // ------------------------------------------------------------------
    // readImmoStatus — read-only UDS ReadDataByIdentifier (ISO-TP routed CAN)
    // ------------------------------------------------------------------

    /**
     * Read-only immobilizer / security-module status via routed ISO-TP UDS service 0x22.
     *
     * Separate from [actuate] — no active tests, writes, or SecurityAccess sequences.
     * Uses the raw CAN shim ([sendRoutedCanFrame] / [awaitRoutedCanData]) for non-standard ID pairs.
     */
    suspend fun readImmoStatus(
        reqCanId: Int,
        respCanId: Int,
        dataIdentifier: Int,
    ): Result<ImmoUdsReadResult> = safeRequest("readImmoStatus") {
        val request = ImmoUdsReadSupport.buildReadRequest(dataIdentifier)
        val tpPayload = exchangeRoutedIsoTp(reqCanId, respCanId, request)
            ?: return@safeRequest Result.failure(
                VciException.ProtocolError(
                    "No ISO-TP response for immo UDS 0x22 DID 0x${
                        dataIdentifier.toString(16).uppercase().padStart(4, '0')
                    } on 0x${reqCanId.toString(16)}→0x${respCanId.toString(16)}",
                ),
            )
        Result.success(
            ImmoUdsReadResult(
    ...
    internal suspend fun exchangeRoutedIsoTp(
        reqCanId: Int,
        respCanId: Int,
        request: ByteArray,
    ): ByteArray? {
        val iso = com.caseforge.scanner.obd.IsoTpHandler(
            emitFlowControl = { fc ->
                sendRoutedCanFrame(reqCanId, fc).getOrThrow()
            },
        )
        for (segment in iso.buildTransmitSequence(request)) {
            sendRoutedCanFrame(reqCanId, segment).getOrThrow()
        }
        while (true) {
            val data = awaitRoutedCanData(respCanId).getOrNull() ?: break
            when (val rx = iso.ingest(data)) {
                com.caseforge.scanner.obd.IsoTpRxResult.NeedMore -> continue
                is com.caseforge.scanner.obd.IsoTpRxResult.Error -> {
                    Log.w(TAG, "readImmoStatus ISO-TP error: ${rx.message}")
                    return null
                }
                is com.caseforge.scanner.obd.IsoTpRxResult.Complete -> return rx.payload
            }
        }
        return null
    }
     ...
     * Skips malformed frames (wrong header / checksum) when [requireValidFraming] is true.
    ...
    private suspend fun awaitResponse(expectedOpcode: KnownOpcode): VciFrame? {
        return try {
            withTimeout(requestTimeoutMs) {
                transport.frames
                    .filter { frame ->
                        if (frame.opcode != expectedOpcode.value) return@filter false
                        if (!requireValidFraming) return@filter true
                        VciProtocolConfig.validateResponseFrame(
                            frame,
                            expectedOpcode = expectedOpcode.value,
                            expectedHeader = expectedHeader,
                        ) is VciProtocolConfig.FrameValidation.Valid
                    }
                    .firstOrNull()
            }
        } catch (_: kotlinx.coroutines.TimeoutCancellationException) {
            Log.w(TAG, "Timeout waiting for ${expectedOpcode.name} (valid framing)")
            null
        }
    }
...
/** Routed ISO-TP payload from a read-only UDS 0x22 immobilizer probe ([VciCommunicator.readImmoStatus]). */
data class ImmoUdsReadResult(
    val dataIdentifier: Int,
    val reqCanId: Int,
    val respCanId: Int,
    val tpPayload: ByteArray,
)

/** Shared UDS 0x22 request builder for immo reads (kept in vci package — no planb dependency). */
internal object ImmoUdsReadSupport {
    fun buildReadRequest(dataIdentifier: Int): ByteArray {
        val hi = (dataIdentifier shr 8) and 0xFF
        val lo = dataIdentifier and 0xFF
        return byteArrayOf(0x22, hi.toByte(), lo.toByte())
    }
}
