...
object CommonSequences {
    ...
    fun relativeCompression(): TestSequence {
        ...
        for (cyl in cylinders) {
            ...
            steps.add(Wait(label = "Settle (Cyl $cyl)", seconds = 2.0))
            steps.add(
                CapturePid(
            ...
            steps.add(Wait(label = "Recover (Cyl $cyl)", seconds = 1.5))
        }
        ...
        return TestSequence(
            ...
            description = "Injector-cutout sweep to measure compression per cylinder.",
            steps = steps,
            timeout = 120_000L,
        )
    }

    fun evapSmokeTest(): TestSequence = TestSequence(
        id = "evap_smoke_test",
        ...
        steps = listOf(
            ...
            RunCapability(label = "Close Vent Valve", capabilityId = "evap_vent_close"),
            Wait(label = "Stabilize", seconds = 1.0),
            CapturePid(
            ...
            Wait(label = "Smoke Dwell", seconds = 30.0),
            CapturePid(
            ...
            Branch(
                ...
                ifFalse = listOf(
                    Prompt(
                        ...
                        message = "EVAP pressure decayed rapidly. Check purge valve, canister seams, and lines.",
                    ),
                ),
            ),
        ),
    ...
    fun parasiticDrawBisection(): TestSequence {
        ...
        steps.add(
            Prompt(
                ...
                message = "Engine off, accessories off. Connect ammeter between battery negative and terminal.",
            ),
        )
        steps.add(
            CapturePid(
        ...
        for ((idx, group) in fuseGroups.withIndex()) {
            ...
            steps.add(Wait(label = "Settle", seconds = 5.0))
            steps.add(
                CapturePid(
            ...
            steps.add(
                Branch(
                    ...
                    condition = "draw_without_group_$idx < 0.05",
                    ifTrue = listOf(
                        Prompt(
                            label = "Culprit Found",
                            message = "Parasitic load is in $group. Reinstall fuses one at a time to isolate the circuit.",
                        ),
                    ),
                    ifFalse = listOf(
                        Prompt(
    ...
    fun vvtSolenoidSweep(): TestSequence = TestSequence(
        ...
        steps = listOf(
            Prompt(
                ...
                message = "VVT solenoid control requires F10 (Direct VCI). Not yet implemented.",
            ),
            Prompt(
                label = "Placeholder Step",
                message = "When F10 is available: command OCV PWM 0–100%, measure cam position delay, detect sticking.",
            ),
        ),
        timeout = 60_000L,
    )
