...
class VehicleDatabaseZipper(
    ...
    /** Extra zip entries, e.g. `harvest-batch/manifest.json` from [TabletDataHarvester]. */
    private val sidecarFiles: Map<String, ByteArray> = emptyMap(),
) {
    ...
    val hasExportableData: Boolean get() = inventory.hasData || sidecarFiles.isNotEmpty()

    val sidecarFileCount: Int get() = sidecarFiles.size

    val sidecarBytes: Long get() = sidecarFiles.values.sumOf { it.size.toLong() }

    fun totalBytesEstimate(): Long = inventory.totalBytes + sidecarBytes

    fun zipProgressFlow(output: OutputStream): Flow<ZipProgress> = flow {
        val inv = inventory
        if (!inv.hasData && sidecarFiles.isEmpty()) throw EmptyVehicleDatabaseException(inv)

        var bytes = 0L
        var files = 0
        val zos = ZipOutputStream(BufferedOutputStream(output, 64 * 1024))
        try {
            sidecarFiles.forEach { (zipPath, content) ->
                files++
