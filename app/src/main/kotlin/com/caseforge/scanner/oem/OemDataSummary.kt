...
data class OemDataSummary(
    ...
    /** From shipped decompile metadata bundle; null when bundle missing or not merged. */
    val decompileBrandCount: Int? = null,
    val decompileCatalogTotals: DecompileCatalogTotals? = null,
    /** Best-effort match between local vehicle tree and reference brand folders. */
    val matchedLocalBrand: String? = null,
) {
    ...
    fun displayLines(): List<String> = buildList {
        ...
        decompileBrandCount?.let { add("Reference inventory: $it vehicle lines (metadata)") }
        decompileCatalogTotals?.let { t ->
            val sum = t.ggp + t.bin + t.bnc
            if (sum > 0) {
                add("Reference catalog totals: GGP ${t.ggp}, BIN ${t.bin}, BNC ${t.bnc}")
            }
        }
        matchedLocalBrand?.let { add("Local vehicle line folder: $it") }
