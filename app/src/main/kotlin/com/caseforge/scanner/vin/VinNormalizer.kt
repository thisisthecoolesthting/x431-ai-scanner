 ...
 *
 * Plan B: [marqueHint] tries WMI heuristics in deterministic order.
 */
object VinNormalizer {
    ...
    private val normalizeToVinCache = lruCache<String, String?>(CACHE_MAX)
    private val marqueHintCache = lruCache<String, String?>(CACHE_MAX)
    ...
    fun normalizeToVin(raw: String): String? =
        synchronized(normalizeToVinCache) {
            if (normalizeToVinCache.containsKey(raw)) {
                return normalizeToVinCache[raw]
            }
            val decoded = pickBestFromOcr(raw)?.normalizedVin?.takeIf { hasValidCharset(it) }
            normalizeToVinCache[raw] = decoded
            decoded
        }

    /** WMI-based marque hint for gateway routing (e.g. wedge candidates); null if unknown. */
    fun marqueHint(vin: String): String? =
        synchronized(marqueHintCache) {
            if (marqueHintCache.containsKey(vin)) {
                return marqueHintCache[vin]
            }
            val hint =
                JeepVinDetector.marqueHint(vin)
                    ?: FordVinDetector.marqueHint(vin)
                    ?: RamVinDetector.marqueHint(vin)
                    ?: ChryslerVinDetector.marqueHint(vin)
                    ?: DodgeVinDetector.marqueHint(vin)
                    ?: GmVinDetector.marqueHint(vin)
                    ?: ToyotaVinDetector.marqueHint(vin)
                    ?: HondaVinDetector.marqueHint(vin)
                    ?: NissanVinDetector.marqueHint(vin)
                    ?: HyundaiVinDetector.marqueHint(vin)
            marqueHintCache[vin] = hint
            hint
        }
    ...
    internal fun clearMemoizedCachesForTest() {
        synchronized(normalizeToVinCache) { normalizeToVinCache.clear() }
        synchronized(marqueHintCache) { marqueHintCache.clear() }
    }

    internal fun cacheSizesForTest(): Pair<Int, Int> =
        synchronized(normalizeToVinCache) {
            synchronized(marqueHintCache) {
                normalizeToVinCache.size to marqueHintCache.size
            }
        }

    private fun <K, V> lruCache(maxEntries: Int): MutableMap<K, V> =
        Collections.synchronizedMap(
            object : LinkedHashMap<K, V>(maxEntries + 1, 0.75f, true) {
