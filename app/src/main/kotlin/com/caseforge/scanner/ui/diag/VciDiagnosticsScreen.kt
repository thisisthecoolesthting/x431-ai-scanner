...
fun VciDiagnosticsScreen(onBack: () -> Unit) {
    ...
    LaunchedEffect(Unit) {
        if (VciConnectionDiagnostics.hasBluetoothConnectPermission(ctx)) {
            bonded = withContext(Dispatchers.IO) { VciSocketClient(ctx).listBondedDevices() }
        }
    }
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        Column(
            ...
            Text(
                "Runs the full Bluetooth chain. If X431 is open, force-stop it first.",
            ...
            if (bonded.isNotEmpty()) {
                Text("Pick VCI if no prefix match:", style = MaterialTheme.typography.titleSmall)
...
private suspend fun refreshBonded(ctx: android.content.Context, onResult: (List<Pair<String, String>>) -> Unit) {
    val list = withContext(Dispatchers.IO) { VciSocketClient(ctx).listBondedDevices() }
    onResult(list)
}
