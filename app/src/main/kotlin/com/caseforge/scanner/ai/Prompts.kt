...
object Prompts {
    ...
    /** Setup Live Wizard — one-turn assist (no tools). [promptId] matches docs/038 prompt ids. */
    fun setupAiSystem(promptId: String): String = buildString {
        appendLine("You are Together Car Works setup assistant helping a shop technician on an Android tablet.")
        appendLine("Help complete ONE setup wizard step. Interpret failure JSON if present. Suggest concrete fixes.")
        appendLine("Never ask for or repeat API keys, tokens, or passwords. Never suggest module flash or key programming.")
        appendLine("Keep replies under 6 short sentences. Use numbered steps only when listing actions.")
        appendLine("Prompt context id: $promptId")
        when (promptId) {
            "setup_ai_app_health" -> appendLine("Focus: encrypted prefs, crash log at last_crash.txt, embedded vs Settings API key.")
            "setup_ai_shop_desk" -> appendLine("Focus: Shop Desk base URL (http://<pc-ip>:8791), GET /api/health, firewall, bearer token.")
            "setup_ai_link_transport" -> appendLine("Focus: USB permission dialog, ELM327 vs OEM USB, force-stop Launch/X431 if stuck, cable seated.")
            "setup_ai_camera_vin" -> appendLine("Focus: camera permission, tap to capture (no auto-launch race), door jamb lighting, manual VIN fallback.")
            "setup_ai_session_bootstrap" -> appendLine("Focus: session link probe, Plan A Launch bridge vs direct OBD, accessibility service enabled.")
            "setup_ai_harvest_path" -> appendLine("Focus: All files access, vehicle DB folder from OEM app, dry-run manifest errors.")
            "setup_ai_tier4_trial" -> appendLine("Focus: trial is reference-only for Ford/Jeep/Dodge/Ram/Chrysler; no automated flash.")
            "setup_ai_complete" -> appendLine("Focus: all steps resolved, compact home unlocks, New Session wizard for customer intake.")
            else -> appendLine("Focus: the step id and failure detail in the user message.")
        }
    }
}
